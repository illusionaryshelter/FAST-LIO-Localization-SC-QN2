...
public:
...

  {
    ...
    std::pair<int, float> sc_detected_ =
        sc_manager_.detectLoopClosureIDGivenScan(
    ...
    if (enable_quatro) {
      ...
      for (int i = dst_idx - submap_range; i < dst_idx + submap_range + 1;
        ...
        if (i >= 0 && i < static_cast<int>(saved_keyframes.size() - 1)) {
          dst_accum += transformPcd(saved_keyframes[i].pcd_,
                                    saved_keyframes[i].pose_eig_);
        }
      }
    }
    return {*voxelizePcd(src_out, voxel_res),
  ...
  RegistrationOutput
  performMapMatcher(const PosePcd &query_keyframe,
                    const std::vector<PosePcdReduced> &saved_keyframes,
    ...
    if (closest_keyframe_idx_ >= 0) {
      ...
      const auto &[src_cloud, dst_cloud] = setSrcAndDstCloud(
          query_keyframe, saved_keyframes, closest_keyframe_idx_,
