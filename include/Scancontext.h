...
using namespace fastlio_nanoflann;
...
public:
  using SCPointType =
      pcl::PointXYZI; // using xyz only. but a user can exchange the original
                      // bin encoding function (i.e., max hegiht) to max
                      // intensity (for detail, refer 20 ICRA Intensity Scan
                      // Context)
  ...
  Eigen::MatrixXd
  makeScancontext(const pcl::PointCloud<SCPointType> &_scan_down);
  ...
  // User-side API
  void
  makeAndSaveScancontextAndKeys(const pcl::PointCloud<SCPointType> &_scan_down);
  std::pair<int, float>
  detectLoopClosureID(void); // int: nearest node index, float: relative yaw
  std::pair<int, float> detectLoopClosureIDGivenScan(
      const pcl::PointCloud<SCPointType>
          &_scan_down); // int: nearest node index, float: relative yaw

  // for ltslam
  // User-side API for multi-session
  void saveScancontextAndKeys(Eigen::MatrixXd _scd);
  std::pair<int, float>
  detectLoopClosureIDBetweenSession(std::vector<float> &curr_key,
...
public:
  ...
  const int NUM_CANDIDATES_FROM_TREE =
      3; // 10 is enough. (refer the IROS 18 paper)
  ...
  double SC_DIST_THRES =
      0.2; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy,
  ...
  const int TREE_MAKING_PERIOD_ =
      30; // i.e., remaking tree frequency, to avoid non-mandatory every
