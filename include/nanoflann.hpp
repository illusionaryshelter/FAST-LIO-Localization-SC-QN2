...
concept has_assign = requires(T a, V v) {
  (void)a.assign(1, v);
};
template <typename T, typename V>
concept can_assign = requires(T a, V v) {
  a[0] = v;
};

template <typename Container>
requires has_resize<Container>
inline void resize(Container &c, const size_t nElements) {
  c.resize(nElements);
}

template <typename Container, typename T>
requires has_assign<Container, T>
inline void assign(Container &c, const size_t nElements, const T &value) {
  c.assign(nElements, value);
}

template <typename Container, typename T>
requires(!has_assign<Container, T> &&
         can_assign<Container, T>) inline void assign(Container &c,
 ...
 * fastlio_nanoflann::metric_L1 \tparam T Type of the elements (e.g. double,
 ...
 * high-dimensionality data sets). Corresponding distance traits:
 * fastlio_nanoflann::metric_L2 \tparam T Type of the elements (e.g. double,
 * float, uint8_t) \tparam _DistanceType Type of distance variables (must be
 * signed) (e.g. float, double, int64_t)
 */
template <class T, class DataSource, typename _DistanceType = T>
 ...
 * fastlio_nanoflann::metric_L2_Simple \tparam T Type of the elements (e.g.
 * double, float, uint8_t) \tparam _DistanceType Type of distance variables
 * (must be signed) (e.g. float, double, int64_t)
 */
template <class T, class DataSource, typename _DistanceType = T>
 ...
 *  Corresponding distance traits: fastlio_nanoflann::metric_SO2
 ...
 *  Corresponding distance traits: fastlio_nanoflann::metric_SO3
 * \tparam T Type of the elements (e.g. double, float)
 * \tparam _DistanceType Type of distance variables (must be signed) (e.g.
 * float, double)
 */
 ...
 * from fastlio_nanoflann::Metric \tparam DIM Dimensionality of data points
 * (e.g. 3 for 3D points) \tparam IndexType Will be typically size_t or int
 */
 ...
 *
 * \tparam DatasetAdaptor The user-provided adaptor (see comments above).
 * \tparam Distance The distance metric to use: fastlio_nanoflann::metric_L1,
 * fastlio_nanoflann::metric_L2, fastlio_nanoflann::metric_L2_Simple, etc.
 * \tparam DIM Dimensionality of data points (e.g. 3 for 3D points) \tparam
...
public:
  ...
  typedef typename fastlio_nanoflann::KDTreeBaseClass<
      fastlio_nanoflann::KDTreeSingleIndexAdaptor<Distance, DatasetAdaptor, DIM,
                                                  IndexType>,
      Distance, DatasetAdaptor, DIM, IndexType>
      BaseClassRef;
   ...
   * https://github.com/jlblancoc/fastlio_nanoflann
   *
  ...
  bool findNeighbors(RESULTSET &result, const ElementType *vec,
    ...
    if (!BaseClassRef::root_node)
      throw std::runtime_error("[fastlio_nanoflann] findNeighbors() called "
  ...
  size_t knnSearch(const ElementType *query_point, const size_t num_closest,
    ...
    fastlio_nanoflann::KNNResultSet<DistanceType, IndexType> resultSet(
        num_closest);
    resultSet.init(out_indices, out_distances_sq);
    this->findNeighbors(resultSet, query_point,
                        fastlio_nanoflann::SearchParams());
    ...
    if (dataset.kdtree_get_bbox(bbox)) {
      ...
      if (!N)
        throw std::runtime_error(
            "[fastlio_nanoflann] computeBoundingBox() called but "
...
public:
  ...
  typedef typename fastlio_nanoflann::KDTreeBaseClass<
      fastlio_nanoflann::KDTreeSingleIndexDynamicAdaptor_<
          Distance, DatasetAdaptor, DIM, IndexType>,
      Distance, DatasetAdaptor, DIM, IndexType>
      BaseClassRef;
 ...
 * Distance The distance metric to use: fastlio_nanoflann::metric_L1,
 * fastlio_nanoflann::metric_L2, fastlio_nanoflann::metric_L2_Simple, etc.
 */
template <class MatrixType, int DIM = -1,
          class Distance = fastlio_nanoflann::metric_L2>
struct KDTreeEigenMatrixAdaptor {
  ...
  KDTreeEigenMatrixAdaptor(const size_t dimensionality,
    ...
    index = new index_t(
        ...
        fastlio_nanoflann::KDTreeSingleIndexAdaptorParams(leaf_max_size));
    index->buildIndex();
  }
...
public:
  ...
  inline void query(const num_t *query_point, const size_t num_closest,
                    ...
                    const int /* nChecks_IGNORED */ = 10) const {
    fastlio_nanoflann::KNNResultSet<num_t, IndexType> resultSet(num_closest);
    resultSet.init(out_indices, out_distances_sq);
