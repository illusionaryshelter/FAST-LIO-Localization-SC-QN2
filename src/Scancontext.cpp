...
{
    std::cout<<"scancontext lib is successfully imported.\n"<<std::endl;
} // coreImportTest
...
std::pair<int, float> SCManager::detectLoopClosureIDBetweenSession (std::vector<float>& _curr_key, Eigen::MatrixXd& _curr_desc)
{
    ...
    fastlio_nanoflann::KNNResultSet<float> knnsearch_result( NUM_CANDIDATES_FROM_TREE );
    knnsearch_result.init( &candidate_indexes[0], &out_dists_sqr[0] );
    polarcontext_tree_batch_->index->findNeighbors( knnsearch_result, &curr_key[0] /* query */, fastlio_nanoflann::SearchParams(10) ); // error here
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    if( tree_making_period_conter % TREE_MAKING_PERIOD_ == 0) // to save computation cost
    {
        ...
        // tree_ptr_->index->buildIndex(); // inernally called in the constructor of InvKeyTree (for detail, refer the fastlio_nanoflann and KDtreeVectorOfVectorsAdaptor)
    ...
    fastlio_nanoflann::KNNResultSet<float> knnsearch_result( NUM_CANDIDATES_FROM_TREE );
    knnsearch_result.init( &candidate_indexes[0], &out_dists_sqr[0] );
    polarcontext_tree_->index->findNeighbors( knnsearch_result, &curr_key[0] /* query */, fastlio_nanoflann::SearchParams(10) ); 
